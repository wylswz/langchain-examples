...
class TagResult(BaseModel):
    """标签识别结果"""
    tags: list[str] = Field(description="识别出的相关标签列表，只能从给定的标签列表中选择")
...
def mark_tags_excel(wb, row_index: int, tags_to_mark: list[str], all_tags: list[str], tag_start_column: int):
    """在 Excel 中标记标签"""
    ws = wb.active
...
def mark_tags_csv(rows: list, row_index: int, tags_to_mark: list[str], all_tags: list[str], tag_start_column: int):
    """在 CSV 数据中标记标签"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="智能标签助手 - 自动为陈述标记相关标签")
    parser.add_argument("file_path", help="输入文件路径（.xlsx, .xls, .csv）")
    parser.add_argument("-o", "--output", default="", help="输出文件路径（默认覆盖原文件）")
    parser.add_argument("-s", "--statement-column", type=int, default=1,
